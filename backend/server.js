// server.js (updated to mount adminRoutes)
...
// Ensure uploads dir exists before serving/writing PDFs
const uploadsDir = path.join(__dirname, 'uploads');
try {
  if (!fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
} catch (err) {
  console.error('Could not ensure uploads directory exists:', err);
}
...
// ---------- try to require models & middleware (non-fatal) ----------
...
function tryRequire(name) {
  try {
    ...
    // return the error to allow custom logging
    return { __requireError: err };
  }
}

const maybeLeave = tryRequire('./models/LeaveApplication');
if (maybeLeave && maybeLeave.__requireError) {
  console.warn('Warning: ./models/LeaveApplication.js not found or errored when requiring.');
  console.warn(maybeLeave.__requireError.stack || maybeLeave.__requireError.message);
} else if (maybeLeave) {
  LeaveApplication = maybeLeave;
  console.log('Loaded models/LeaveApplication.js');
}

const maybeUser = tryRequire('./models/User');
if (maybeUser && maybeUser.__requireError) {
  // optional model - log quietly
  console.warn('Note: ./models/User.js not loaded (optional).');
} else if (maybeUser) {
  User = maybeUser;
...
const maybeAuth = tryRequire('./middleware/authMiddleware');
if (maybeAuth && maybeAuth.__requireError) {
  console.warn('Warning: ./middleware/authMiddleware.js not found or errored when requiring. Using dev fallback auth middleware.');
  // VERY simple fallback for dev testing only — remove or replace in production
  authMiddleware = (req, res, next) => {
    const { userId, role, name } = req.query;
    if (req.headers && req.headers.authorization && req.headers.authorization.startsWith('Bearer ')) {
      // If someone mounts a real JWT middleware later, it will replace this.
      // For now, allow headers-based bearer tokens only if an external auth middleware is present.
    }
    if (userId && role) {
      req.user = { id: userId, role, name: name || undefined };
      return next();
    }
    return res.status(401).json({ message: 'Missing auth middleware and no test query params provided.' });
  };
} else {
  authMiddleware = maybeAuth;
  console.log('Loaded middleware/authMiddleware.js');
}

// Role middlewares (with fallbacks)
const maybeStudentOnly = tryRequire('./middleware/studentOnly');
if (maybeStudentOnly && maybeStudentOnly.__requireError) {
  console.warn('Warning: ./middleware/studentOnly.js not found — using simple fallback.');
  ...
  studentOnly = maybeStudentOnly;
  console.log('Loaded middleware/studentOnly.js');
}

const maybeFacultyOnly = tryRequire('./middleware/facultyOnly');
if (maybeFacultyOnly && maybeFacultyOnly.__requireError) {
  console.warn('Warning: ./middleware/facultyOnly.js not found — using simple fallback.');
  ...
  facultyOnly = maybeFacultyOnly;
  console.log('Loaded middleware/facultyOnly.js');
}

const maybeRectorOnly = tryRequire('./middleware/rectorOnly');
if (maybeRectorOnly && maybeRectorOnly.__requireError) {
  console.warn('Warning: ./middleware/rectorOnly.js not found — using simple fallback.');
...
function tryMountRoute(routePath, mountPoint) {
  const maybe = tryRequire(routePath);
  if (maybe && maybe.__requireError) {
    console.warn(`routes ${routePath} not found or errored; skipping. Error:`, maybe.__requireError.message);
    return false;
  }
  if (maybe) {
    app.use(mountPoint, maybe);
    mountedAtLeastOneRoute = true;
    mountedRoutes.push({ mountPoint, file: routePath });
...
tryMountRoute('./routes/authRoutes', '/api/auth');
tryMountRoute('./routes/facultyRoutes', '/api/faculty');
tryMountRoute('./routes/rectorRoutes', '/api/rector');
tryMountRoute('./routes/leaveRoutes', '/api/leaves');

// <-- NEW: attempt to mount adminRoutes (regenerate PDF endpoint)
tryMountRoute('./routes/adminRoutes', '/api/admin');
...
fallbackRouter.patch('/faculty/approve/:leaveId', authMiddleware, facultyOnly, async (req, res) => {
  try {
    ...
    const action = (req.body && req.body.action) || req.query.action || 'approve';
    ...
    try {
      leave.status = typeof recomputeOverallStatus === 'function' ? recomputeOverallStatus(leave) :
                    (leave.facultyStatus === 'approved' && (leave.rectorStatus === 'approved') ? 'approved' :
...
// Generic role-aware status endpoint
...
// Mount fallback router at /api if no external routes were mounted,
// otherwise expose fallback at /api/fallback so it can be used intentionally.
if (!mountedAtLeastOneRoute) {
  app.use('/api', fallbackRouter);
  console.log('No external route files mounted — fallback handlers are active at /api/*');
} else {
  app.use('/api/fallback', fallbackRouter);
  console.log('Some external routes mounted. Fallback handlers are available at /api/fallback/*');
}

// Root health
app.get('/', (req, res) => res.json({ message: 'Leave management backend is running' }));

// Helper: print mounted routes (improved to show router prefixes)
function printRoutes() {
  ...
  function walk(stack, prefix = '') {
    stack.forEach((layer) => {
      if (layer.route) {
        ...
        const mountPath = layer.regexp && layer.regexp.fast_slash ? prefix || '' : prefix || '';
        // iterate router stack
        walk(layer.handle.stack, prefix + (layer.regexp && layer.regexp.fast_slash ? '' : ''));
      } else if (layer.name && layer.name !== 'bound dispatch' && layer.handle && layer.handle.stack) {
        // another router-like layer
  ...
  // Simple pass to print top-level routes (best-effort)
  try {
    app._router.stack.forEach((middleware) => {
      if (middleware.route) {
        ...
        // show the router's internal routes with no accurate prefix (best-effort)
    ...
    console.warn('printRoutes encountered an error while enumerating routes:', err && err.message);
  }
  ...
  try {
    ...
    app.listen(PORT, () => {
      ...
      if (mountedRoutes.length) {
        console.log('Mounted external route files:');
        mountedRoutes.forEach(r => console.log(` - ${r.mountPoint} -> ${r.file}`));
      }
    });
  } catch (err) {
    console.error('Failed to connect to MongoDB or start server:', err && (err.stack || err.message || err));
    process.exit(1);
  }
}
